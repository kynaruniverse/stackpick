/* ============================================================
   ...
   05  Patch builder (story-strip + patch-rail)
   06  Wall renderer (renderCollection)
   07  Collection switcher (switchCollection)
   08  Loadout cluster injector
   09  Sort menu
   10  Scroll — parallax shadow + patch jiggle
   11  Pull-to-shuffle gesture
   12  Shuffle banner
...
(function () {
  ...
  function applyTheme(theme) {
    ...
    if (railBtn) {
      var rs = railBtn.querySelector('.patch-rail__theme-icon');
      if (rs) rs.textContent = icon;
    }
  ...
  function initPrefsSheet() {
    ...
    function openSheet() {
      sheet.classList.add('prefs-sheet--open');
    ...
    function closeSheet() {
      sheet.classList.remove('prefs-sheet--open');
  ...
  var _rackSheet   = null;
  var _rackOverlay = null;

  function closeRackSheet() {
    if (!_rackSheet) return;
    _rackSheet.classList.remove('rack-sheet--open');
    _rackOverlay.classList.remove('open');
    _rackSheet.setAttribute('aria-hidden', 'true');
    _rackOverlay.setAttribute('aria-hidden', 'true');
  }
  ...
  var RACK_CONTENT = {
    browse: [
      ...
      { label: '\uD83D\uDDB1\uFE0F Mice', href: '/mice/' },
      { label: '\uD83D\uDDA5\uFE0F Monitors', href: '/monitors/' },
      { label: '\uD83E\uDE91 Chairs',    href: '/chairs/' },
    ],
    loadouts: [
      { label: '\uD83D\uDCCB Setup Guides',    href: '/guides/' },
      { label: '\u2696\uFE0F Comparisons',     href: '/comparisons/' },
      { label: '\uD83C\uDFAE \u00A3500 Build', href: '/guides/gaming-setup-500/' },
      { label: '\uD83D\uDD25 \u00A31000 Build',href: '/guides/gaming-setup-1000/' },
      { label: '\uD83D\uDC8E \u00A32500 Build',href: '/guides/gaming-setup-2500/' },
    ],
    drops: [
      { label: '\u26A1 Latest Picks',       href: '/comparisons/' },
      { label: '\u2696\uFE0F Mouse Showdown', href: '/comparisons/logitech-superlight-2-vs-razer-viper-v3-pro/' },
      { label: '\uD83D\uDDA5\uFE0F Monitor Battle', href: '/comparisons/asus-pg27aqdp-vs-pg32ucdm/' },
      { label: '\uD83E\uDE91 Chair Face-Off', href: '/comparisons/secretlab-titan-evo-vs-herman-miller-aeron/' },
    ],
    profile: [
      { label: '\u2139\uFE0F About StackPick', href: '/about/' },
  ...
  function buildRackHTML(tab) {
    var items = RACK_CONTENT[tab] || [];
    var title = RACK_TITLES[tab] || tab;
    var h = '<p style="font-family:var(--font-display);font-size:1.1rem;font-weight:800;'
          + 'text-transform:uppercase;letter-spacing:0.08em;color:var(--text-tertiary);'
          + 'margin-bottom:var(--sp-3);">' + title + '</p>';
    return h + items.map(function (it) {
      return '<a href="' + it.href + '" style="display:block;padding:var(--sp-3) var(--sp-4);'
           ...
           + 'font-weight:500;color:var(--text-primary);text-decoration:none;margin-bottom:var(--sp-2);">'
           + it.label + '</a>';
    }).join('');
  }

  function initRack() {
    _rackSheet   = document.getElementById('rack-sheet');
    _rackOverlay = document.getElementById('rack-sheet-overlay');
    var boxes = document.querySelectorAll('.rack-box');
    var holdTimer = null;
    ...
    boxes.forEach(function (box) {
      ...
      box.addEventListener('click', function () {
        ...
        boxes.forEach(function (b) {
          b.classList.toggle('rack-box--active', b === box);
          b.setAttribute('aria-pressed', b === box ? 'true' : 'false');
        });
      });

      box.addEventListener('pointerdown', function () {
        holdTimer = setTimeout(function () {
          box.classList.add('rack-box--lifted');
          var content = document.getElementById('rack-sheet-content');
          if (content) content.innerHTML = buildRackHTML(tab);
          if (_rackSheet) {
            _rackSheet.classList.add('rack-sheet--open');
            _rackSheet.setAttribute('aria-hidden', 'false');
          }
          if (_rackOverlay) {
            _rackOverlay.classList.add('open');
            _rackOverlay.setAttribute('aria-hidden', 'false');
          }
          setTimeout(function () { box.classList.remove('rack-box--lifted'); }, 400);
        }, 400);
      });
      box.addEventListener('pointerup',    function () { clearTimeout(holdTimer); });
      box.addEventListener('pointerleave', function () { clearTimeout(holdTimer); });
  ...
  function buildSpecs(specs) {
    return (specs || []).map(function (s) {
      return '<span class="spec-pill">' + escapeHTML(s) + '</span>';
    }).join('');
  }
  ...
  function buildLoadoutDots(count) {
    var MAX = 5;
    var n = Math.min(Math.max(count || 0, 0), MAX);
    var dots = '';
    for (var i = 0; i < MAX; i++) {
      dots += '<span class="loadout-dot' + (i < n ? ' loadout-dot--filled' : '') + '" aria-hidden="true"></span>';
    }
  ...
  function buildDrawerStats(product) {
    var rows = '';
    (product.pros || []).forEach(function (pro, i) {
      rows += '<div class="drawer-stat"><span class="drawer-stat__key">' + (i === 0 ? 'Pros' : '') + '</span>'
            + '<span class="drawer-stat__val">\u2713\u00A0' + escapeHTML(pro) + '</span></div>';
    });
    (product.cons || []).forEach(function (con) {
      rows += '<div class="drawer-stat drawer-stat--con"><span class="drawer-stat__key">Watch</span>'
            + '<span class="drawer-stat__val">\u2715\u00A0' + escapeHTML(con) + '</span></div>';
    });
    return rows;
  }
  ...
  function buildAltPicks(product) {
    ...
    var alts = window.SP_PRODUCTS.filter(function (p) {
    ...
    return alts.map(function (alt) {
      return '<li class="alt-pick">'
           + '<span class="alt-pick__name">' + escapeHTML(alt.shortName) + '</span>'
           + '<span class="alt-pick__price">' + escapeHTML(alt.price) + '</span>'
           + '<a class="alt-pick__link" href="' + alt.affiliate + '" target="_blank" rel="noopener sponsored"'
           + ' aria-label="View ' + escapeHTML(alt.name) + ' on Amazon UK">\u2192</a>'
           + '</li>';
    }).join('');
  }
  ...
  function buildCard(product, index, animDelay) {
        ...
        '<div class="box-card__front">',
          ...
          '<div class="box-card__drawer" aria-hidden="true">',
            ...
            '<div class="box-card__drawer-price">',
              ...
              '<a class="box-card__cta" href="' + product.affiliate + '" target="_blank" rel="noopener sponsored"'
                + ' aria-label="View ' + escapeHTML(product.name) + ' on Amazon UK">View on Amazon \u2192</a>',
            '</div>',
          '</div>',
        '</div>',
    ...
    front.addEventListener('click', function (e) {
      ...
      if (typeof gtag === 'function') gtag('event', 'card_expand', {
        product_id: product.id, product_name: product.name, category: product.category, expanded: expanded,
      });
    });
    ...
    flip.addEventListener('click', function (e) {
      ...
      if (typeof gtag === 'function') gtag('event', 'card_flip', {
        product_id: product.id, product_name: product.name, flipped: flipped,
      });
    });

    article.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); front.click(); }
    });

    article.addEventListener('animationend', function () {
      article.classList.remove('box-card--entering');
    }, { once: true });
  ...
  function wirePatch(patchEl, col, preview, previewCards, previewCta) {
    ...
    patchEl.addEventListener('pointerdown', function () {
      ...
      _longPressTimer = setTimeout(function () {
        ...
        previewCards.innerHTML = products.map(function (p) {
          ...
          return '<div style="flex:1;min-width:0;background:var(--surface-card);border:1px solid var(--border-strong);'
               + 'border-radius:8px;padding:var(--sp-3);text-align:center;border-right:3px solid ' + seam + ';">'
               + '<div style="font-size:28px;margin-bottom:4px;">' + p.emoji + '</div>'
               + '<div style="font-family:var(--font-display);font-size:10px;font-weight:800;text-transform:uppercase;'
               + 'letter-spacing:0.06em;color:var(--text-primary);line-height:1.1;">' + escapeHTML(p.shortName) + '</div>'
               + '<div style="font-family:var(--font-mono);font-size:9px;color:var(--text-tertiary);margin-top:2px;">' + escapeHTML(p.price) + '</div>'
               + '</div>';
        }).join('');
    ...
    patchEl.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); patchEl.click(); }
    });
  ...
  function switchCollection(collectionId) {
    ...
    setTimeout(function () {
      ...
      updatePatchActiveStates(collectionId);
      updateWallHeader(collectionId, products.length);
      closeSortMenu();
      var wall = document.getElementById('wall');
      if (wall) wall.scrollIntoView({ behavior: 'smooth', block: 'start' });
      html.classList.remove('wall-body--transitioning');
  ...
  function buildLoadoutCluster(products) {
    ...
    var miniBoxes = display.map(function (p, i) {
      var seam = SEAM_COLOURS[p.category] || '#8E8EA0';
      return '<div class="loadout-cluster__mini-box loadout-cluster__mini-box--' + (i + 1) + '" style="border-right:2px solid ' + seam + ';">'
           + '<span class="mini-box__silhouette">' + p.emoji + '</span>'
           + '</div>';
    }).join('');

    section.innerHTML =
      '<div class="loadout-cluster__stack">' + miniBoxes + '</div>'
    + '<div class="loadout-cluster__meta">'
    +   '<h3 class="loadout-cluster__title">' + escapeHTML(title) + '</h3>'
    +   '<span class="loadout-cluster__items">' + products.length + ' items in range</span>'
    +   '<span class="loadout-cluster__total">' + escapeHTML(totalFmt) + ' combined</span>'
    +   '<a class="loadout-cluster__cta" href="/guides/">See Setup Guides \u2192</a>'
    + '</div>';
  ...
  function closeSortMenu() {
    var menu    = document.getElementById('wall-sort-menu');
    var sortBtn = document.getElementById('wall-sort');
    if (!menu) return;
  ...
  function initSortMenu() {
    ...
    menu.querySelectorAll('.wall__sort-option').forEach(function (opt) {
      opt.addEventListener('click', function () {
        ...
        menu.querySelectorAll('.wall__sort-option').forEach(function (o) {
          o.classList.toggle('wall__sort-option--active', o === opt);
          o.setAttribute('aria-selected', o === opt ? 'true' : 'false');
        });
        closeSortMenu();
        var products = state.shuffleStep > 0
          ? SP_getShuffleVariant(state.activeCollectionId, state.shuffleStep - 1)
          : SP_getCollectionProducts(state.activeCollectionId);
        renderCollection(products);
      });
    });

    document.addEventListener('click', function (e) {
      if (menu && !menu.contains(e.target) && e.target !== sortBtn) closeSortMenu();
    });
  }


  /* ══════════════════════════════════════════════════════════
     10  SCROLL — parallax + patch jiggle
     ══════════════════════════════════════════════════════════ */
    ...
    window.addEventListener('scroll', function () {
      ...
      window.requestAnimationFrame(function () {
        ...
        document.querySelectorAll('.box-card__shadow-layer').forEach(function (l) {
          l.style.transform = 'translateY(' + (scrollY * 0.04) + 'px)';
        });
  ...
  function initPullToShuffle() {
    ...
    var startY = 0, pulling = false, THRESHOLD = 80, SHOW_AT = 30;
    ...
    document.addEventListener('touchmove', function (e) {
      ...
      if (delta > SHOW_AT) {
        ...
        stack.style.transform = 'translateY(' + Math.min(delta * 0.3, 24) + 'px)';
      }
    }, { passive: true });

    document.addEventListener('touchend', function (e) {
      if (!pulling) return;
      var delta = (e.changedTouches[0] || { clientY: startY }).clientY - startY;
      pulling = false;
      trigger.classList.remove('shuffle-trigger--pulling');
      stack.style.transform = '';
      if (delta >= THRESHOLD) triggerShuffle();
    }, { passive: true });
  }
  ...
  function triggerShuffle() {
    ...
    var stack = document.getElementById('card-stack');
    if (stack) {
      stack.style.transition = 'transform 200ms ease, opacity 200ms ease';
      stack.style.transform  = 'scaleY(0.96) translateY(-4px)';
      stack.style.opacity    = '0.7';
      setTimeout(function () {
        renderCollection(products);
        stack.style.transition = 'transform 300ms var(--ease-spring), opacity 300ms ease';
        stack.style.transform  = '';
        stack.style.opacity    = '';
        setTimeout(function () { stack.style.transition = ''; }, 350);
      }, 220);
    } else {
      renderCollection(products);
    }
    showShuffleBanner(col.label + ' \u2014 ' + (variant.label || 'v' + state.shuffleStep));
  }
  ...
  function showShuffleBanner(text) {
    var banner = document.getElementById('shuffle-banner');
    var label  = document.getElementById('shuffle-banner-text');
    if (!banner) return;
    clearTimeout(_shuffleTimer);
    if (label) label.textContent = 'Shuffled: ' + text;
    banner.classList.add('shuffle-banner--visible');
    _shuffleTimer = setTimeout(hideSufflesBanner, 2500);
  }

  function hideSufflesBanner() {
    var b = document.getElementById('shuffle-banner');
    if (b) b.classList.remove('shuffle-banner--visible');
    clearTimeout(_shuffleTimer);
  }
  ...
  function initShuffleBanner() {
    ...
    btn.addEventListener('click', function () {
      ...
      html.setAttribute('data-shuffle-step', '0');
      hideSufflesBanner();
      renderCollection(SP_getCollectionProducts(state.activeCollectionId));
    });
  }
  ...
  function initPWABanner() {
    ...
    function show() {
      ...
      s.textContent = '#sp-pwa{position:fixed;bottom:calc(var(--utility-rack-h)+8px);left:0;right:0;z-index:var(--z-banner);padding:0 1rem;pointer-events:none;}'
                    + '@media(min-width:768px){#sp-pwa{bottom:1rem;max-width:480px;left:50%;transform:translateX(-50%);}}'
                    + '.sp-pwa-i{background:var(--surface-lift);border:1px solid var(--border-strong);border-radius:12px;padding:.85rem 1rem;display:flex;align-items:center;gap:.75rem;box-shadow:var(--shadow-card);pointer-events:all;}'
                    + '.sp-pwa-t{flex:1;font-size:.875rem;line-height:1.4;color:var(--text-primary);font-family:var(--font-body);}'
                    + '.sp-pwa-a{display:flex;gap:.5rem;align-items:center;flex-shrink:0;}'
                    + '.sp-pwa-btn{background:var(--volt);color:var(--volt-text);border:none;border-radius:8px;padding:.45rem .9rem;font-size:.75rem;font-weight:800;cursor:pointer;font-family:var(--font-display);letter-spacing:.06em;text-transform:uppercase;}'
                    + '.sp-pwa-x{background:transparent;color:var(--text-tertiary);border:none;cursor:pointer;font-size:1rem;padding:.25rem .5rem;}';
      ...
      b.querySelector('.sp-pwa-btn').addEventListener('click', function () {
        if (deferred) {
          deferred.prompt();
          deferred.userChoice.then(function (r) { if (typeof gtag === 'function') gtag('event','pwa_install_prompt',{outcome:r.outcome}); deferred=null; b.remove(); });
        } else {
          var m = document.getElementById('sp-pwa-m');
          if (m) m.innerHTML = 'Tap <strong>Share</strong> then <strong>Add to Home Screen</strong>.';
          b.querySelector('.sp-pwa-btn').style.display = 'none';
        }
      });
  ...
  function init() {
    ...
    renderCollection(products);
    updateWallHeader(state.activeCollectionId, products.length);
    ...
    console.log(
      '%c\u26A1 StackPick%c 6C \u2014 ' + SP_PRODUCTS.length + ' products, ' + SP_COLLECTIONS.length + ' collections loaded',
      'color:#C8FF00;font-weight:800;font-family:monospace;font-size:13px;',
      'color:#8E8EA0;font-family:monospace;font-size:13px;'
    );
  }
