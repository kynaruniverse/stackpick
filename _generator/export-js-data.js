...
/**
 * StackPick v2 — export-js-data.js
 *
 ...
 *         _data/guides.json
 *
 * Writes: assets/js/data/products.js      → window.SP_PRODUCTS
 *         assets/js/data/collections.js   → window.SP_COLLECTIONS
 *         assets/js/data/search-index.js  → window.SP_SEARCH_INDEX
 *
 * CONSUMERS:
 *   index.html         loads products.js + collections.js (wall.js reads both globals)
 *   search/index.html  loads search-index.js (app.js reads SP_SEARCH_INDEX)
 *
 * NOTE: index.html must load BOTH data files, not a single merged data.js.
 *   Correct script tags in index.html:
 *     <script src="/assets/js/data/products.js" defer></script>
 *     <script src="/assets/js/data/collections.js" defer></script>
 *     <script src="/assets/js/wall.js" defer></script>
 *
 *   Correct script tags in search/index.html:
 *     <script src="/assets/js/data/search-index.js" defer></script>
 *     <script src="/assets/js/app.js" defer></script>
 *
 * All three output files are auto-generated — DO NOT edit by hand.
 * Edit the _data/*.json source files instead, then run this script.
 *
...
// ---------------------------------------------------------------------------
// Export products.js
// Consumed by: index.html via wall.js (window.SP_PRODUCTS)
// ---------------------------------------------------------------------------
...
// Consumed by: index.html via wall.js (window.SP_COLLECTIONS)
//
// v2 note: collection label display names are mapped client-side in wall.js
// via COLLECTION_LABELS. The "label" fields in collections.json are the
// original source names — no need to update the JSON for the v2 rename.
// ---------------------------------------------------------------------------
...
// Consumed by: search/index.html via app.js (window.SP_SEARCH_INDEX)
//
...
function exportSearchIndex(products, timestamp) {
  ...
  products.forEach(p => {
    ...
    // Rich tags string from all searchable fields
    const tags = [
      p.brand,
      p.name,
      p.shortName,
  ...
  guides.forEach(g => {
    ...
    // FIX (carried from v1): guides.json uses "budgetLabel", not "budget".
    // Using g.budget always resolved to undefined, silently dropping budget
    // from search tags and returning price: null on all guide entries.
    const tags = [
      g.title,
      g.budgetLabel     || '',
      g.metaDescription || '',
...
function run() {
  ...
  // Single timestamp so all output files are consistent
  ...
  try {
    if (!products) {
      // products export failed — load directly so search index can still build
  ...
  // Propagate failure — a partial export leaves stale/mismatched files.
  // build.js must not continue if any export failed.
  if (failed > 0) {
